                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <form @submit.prevent="onSubmit" ref="createDeposit" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label for="exampleInputEmail1">Monto Depósitado. Te falta por depositar <strong>$ {{ formatCollectionAmount(collection_total_amount - deposit_total_amount) }}</strong></label>
...
<script>
    export default {
        ...
        methods: {
            ...
            getRemainingAmount() {
                axios.get('/api/collection/amount/'+this.form.branch_office_id+'/'+this.form.collection_date+'?api_token='+App.apiToken)
                .then(response => {
                ...
                axios.get('/api/deposit/amount/'+this.form.branch_office_id+'/'+this.form.collection_date+'?api_token='+App.apiToken)
                .then(response => {
                    this.deposit_total_amount = response.data.data;

                    alert(this.deposit_total_amount);
                });
            },
            onFileChange(e){
